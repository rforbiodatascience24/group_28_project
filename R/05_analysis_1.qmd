...
```
We can see that most patients are diagnosed in stage IIA and stage IIB, 
which is also the stages with the most patients surviving.
...
```{r}
model <- lm(days_to_death ~ cancer_stage + age_at_diagnosis + prior_malignancy + treatment + primary_diagnosis, data = data) 
summary(model)
```